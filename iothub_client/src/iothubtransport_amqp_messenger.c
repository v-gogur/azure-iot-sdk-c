...
static STRING_HANDLE create_devices_path(STRING_HANDLE iothub_host_fqdn, STRING_HANDLE device_id)
{
	...
	if (STRING_sprintf(devices_path, IOTHUB_DEVICES_PATH_FMT, STRING_c_str(iothub_host_fqdn), STRING_c_str(device_id)) != RESULT_OK)
	{
...
static STRING_HANDLE create_unique_tag(const char* prefix, const char* infix)
{
	char* unique_id = NULL;
	STRING_HANDLE tag = NULL;

	if (UniqueId_Generate(unique_id, 8) != UNIQUEID_OK)
	{
...
{
#if defined(IOTHUB_SDK_VERSION) && defined(IOTHUB_SDK_VERSION)
...
static int create_event_sender(MESSENGER_INSTANCE* instance)
{
	...
	else if ((link_name = create_unique_tag(MESSAGE_SENDER_LINK_NAME_PREFIX, STRING_c_str(instance->device_id))) == NULL)
	{
...
static int create_message_receiver(MESSENGER_INSTANCE* instance)
{
	...
	else if ((link_name = create_unique_tag(MESSAGE_RECEIVER_LINK_NAME_PREFIX, STRING_c_str(instance->device_id))) == NULL)
	{
...
static int send_pending_events(MESSENGER_INSTANCE* instance)
{
	...
	while ((message = get_next_event_to_send(instance)) != NULL)
	{
		...
		if ((api_result = message_create_from_iothub_message(message->messageHandle, &amqp_message)) != RESULT_OK)
		{
			...
			// Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_103: [If message_create_from_iothub_message() fails, messender_do_work() shall fail and return]
...
MESSENGER_HANDLE messenger_create(const MESSENGER_CONFIG* messenger_config)
{
	...
	else
	{
		...
		else if ((instance->device_id = STRING_construct(messenger_config->device_id)) == NULL)
		{
